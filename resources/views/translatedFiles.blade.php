    ...
    <section class="space-ptb course-details">
      <div class="container">
        <div class="row">
          ...
          <?php
            // $translated->file ='public/files to translate/test.pdf';
            //  dd($translated->file);
          ?>
          @if(isset($translated->file))
          

          <embed type="text/html" src='{{asset("TranslatedFiles/". $translated->file)}}'  width="100%" height="100%">

            @elseif(count($translated->transImages) == 1)
            <img class="img-fluid  mb-4" src="images/{{$translated->transImages[0]->image}}" alt="">
            @else


            <!--=================================
    Banner -->
    <section class="slider-01" style='height: 555px;'>
        <div class="container-fluid px-0">
          <div id="main-slider" class="swiper-container">
            <div class="swiper-wrapper">
                @foreach ($translated->transImages as $index=>$s)
                
                <div class="swiper-slide slide-0{{$index}} align-items-center d-flex bg-overlay-black-50 header-position"  style="background-image: url({{asset('images/'.$s->image)}}); background-size: cover; background-position: center; background-repeat: no-repeat; height: 555px;">
                    <div class="container">
                      <div class="row">
                        <div class="col-md-12">
        ...
        <h4 class="news-title mb-2">{{$translated->title }}</h4>
            ...
            @endif
              <div class="p-4 border-top"id='order'>
                <form class="form-flat-style" action="{{route('payment')}}" method='post'>
                  @csrf
                  <div class="form-row">
               ...
               <input type="hidden" class="form-control"   id='total_ip' name="total" >
                ...
                <div class="form-group col-sm-6">
                  ...
                  <!-- address -->
                  <div class="form-group col-sm-12" id='deliver_address'>
            ...
            <div class="widgets">
              <div class="widget widget-price">
                ...
                <div class="p-4 border-top">
                      ...
                      <span class="lead text-dark fw-6">Status</span>
                      <p class="mt-2">{{$translated->translates->status ?? ''}} </p>
                    </div>
                </div>
              </div>
              <!-- <div class="widget">
    ...
    $('#copy_no').val(0)
        ...
        //  alert('hi');  
            ...
            $('#price').click(function(){
               ...
               if(( type == 'deliver') && (lang_price != 0) && (copy_no != 0) && (from != 0)&& (to != 0)){
                var price =  parseInt(city_price)+(lang_price*copy_no*(to - from + 1));
                $('#total_ip').val(price);
                $('#total').html('<h5>'+price+' LE</h5>');
             
               }else if(( type == 'pick-up') && (lang_price != 0) && (copy_no != 0) && (from != 0)&& (to != 0)){
                var price = parseInt(office_price)+(lang_price*copy_no*(to - from + 1));
                $('#total_ip').val(price);
                $('#total').html('<h5>'+price+' LE</h5>');
